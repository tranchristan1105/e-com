...
import { 
  ...
  ArrowLeft, Share2, Heart, Check, Plus, Minus, RefreshCw, AlertTriangle, ArrowRight, Zap, Gem, Droplets, Flame, Shield
} from 'lucide-react';
...
const formatPrice = (price) => {
  try { return new Intl.NumberFormat('fr-FR', { style: 'currency', currency: 'EUR' }).format(Number(price) || 0); } catch (e) { return "0,00 €"; }
};

// --- COMPOSANTS UI DARK LUXE ---

const TechSpec = ({ icon: Icon, label, value }) => (
  <div className="flex flex-col p-4 bg-white/5 backdrop-blur-sm rounded-sm border border-white/10 hover:border-yellow-600/50 transition-all duration-300 group cursor-default">
...
const ColorSwatch = ({ color, active, onClick }) => (
  <button 
    onClick={onClick} 
    className={`w-10 h-10 rounded-full border p-1 transition-all duration-300 ${active ? 'border-yellow-500 scale-110' : 'border-white/20 hover:border-white/50'}`}
  >
...
const ProductPage = () => {
  ...
  const [selectedSize, setSelectedSize] = useState('Chef (XL)');
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        console.warn("Mode Fallback activé (Copywriting Luxe)");
        setProduct({
            id: 1,
            name: "L'Héritage Titan™ - Édition Chef",
            price: 89.90,
            category: "Excellence Culinaire",
            description: "Cessez de cuisiner sur des nids à bactéries. Le Titane est le seul matériau au monde biocompatible, inaltérable et éternel. Adoptez l'hygiène absolue.",
            image_url: "https://images.unsplash.com/photo-1628103598586-b4d216f40396?q=80&w=1200"
        });
        setRelatedProducts([
            {id: 99, name: "Le Damas Noir (Couteau)", price: 149.00, image_url: "https://images.unsplash.com/photo-1593642632823-8f78536788c6?w=800"},
            {id: 98, name: "Set Sommelier Or", price: 59.90, image_url: "https://images.unsplash.com/photo-1585553616435-2dc0a54e271d?w=800"}
        ]);
      } finally {
        setLoading(false);
      }
    ...
    <div className="bg-[#0c0a09] min-h-screen font-sans text-gray-200 selection:bg-yellow-600 selection:text-white pb-32">
      ...
      <nav className="fixed w-full z-50 top-0 left-0 bg-[#0c0a09]/80 backdrop-blur-md border-b border-white/5">
        <div className="max-w-[1800px] mx-auto px-6 h-20 flex items-center justify-between">
            ...
            <span className="font-bold text-lg tracking-tight hidden md:block text-white">{product.name}</span>
      ...
      <div className="pt-24 max-w-[1600px] mx-auto px-4 md:px-8">
        <div className="flex flex-col lg:flex-row gap-12 xl:gap-24">
            ...
            <div className="lg:w-7/12 xl:w-2/3 flex flex-col gap-6">
                ...
                {/* --- NOUVELLE SECTION COPYWRITING : POURQUOI LE TITANE ? --- */}
                <div className="mt-16 border-t border-white/5 pt-16">
                    <h2 className="font-serif text-3xl md:text-4xl text-white mb-10 tracking-tight">Le Titane : <span className="text-yellow-600 italic">L'Âge d'Or de la Cuisine.</span></h2>
                    
                    <div className="grid md:grid-cols-2 gap-12">
                        <div className="space-y-6">
                            <div className="flex gap-4">
                                <div className="mt-1 bg-yellow-600/10 p-3 rounded-sm h-fit border border-yellow-600/20">
                                    <ShieldCheck className="text-yellow-500" size={24} />
                                </div>
                                <div>
                                    <h3 className="font-bold text-white uppercase tracking-wider text-sm mb-2">100% Hygiénique</h3>
                                    <p className="text-gray-400 text-sm leading-relaxed">
                                        Le bois absorbe les jus. Le plastique garde les entailles. Le Titane est <strong className="text-white">non-poreux</strong>. Aucune bactérie, aucune odeur, aucun résidu ne peut s'y incruster. Un simple rinçage suffit pour retrouver une surface stérile.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-white uppercase tracking-wider text-sm mb-2">Inaltérable & Éternel</h3>
                                    <p className="text-gray-400 text-sm leading-relaxed">
                                        Plus dur que l'acier inoxydable classique, le titane grade 1 ne rouille pas, ne se tache pas et résiste aux acides (citron, vinaigre). C'est un investissement unique pour toute une vie.
                                    </p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="space-y-6">
                            <div className="flex gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-white uppercase tracking-wider text-sm mb-2">Respect de vos Couteaux</h3>
                                    <p className="text-gray-400 text-sm leading-relaxed">
                                        Contrairement à la pierre ou au verre qui détruisent le fil des lames, le titane offre une dureté calibrée pour résister aux coups tout en préservant le tranchant de vos couteaux japonais.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div className="flex gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-white uppercase tracking-wider text-sm mb-2">Décongélation Rapide</h3>
                                    <p className="text-gray-400 text-sm leading-relaxed">
                                        Propriété méconnue : le titane est un excellent conducteur thermique. Posez un steak congelé dessus, et il décongèlera 3x plus vite qu'à l'air libre, sans cuisson ni électricité.
                                    </p>
                                </div>
                            </div>
                        </div>
            ...
            <div className="lg:w-5/12 xl:w-1/3 flex flex-col relative">
                <div className="lg:sticky lg:top-32 space-y-10">
                    ...
                    <div className="space-y-6 border-b border-white/10 pb-8">
                        <div className="flex items-center gap-2 mb-2">
                             <span className="text-xs font-bold text-yellow-600 uppercase tracking-widest flex items-center gap-1">
                                <Star size={12} fill="currentColor"/> Best-Seller 2025
                            </span>
                        </div>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="pt-4">
                           <span className="text-xs font-bold uppercase tracking-[0.2em] text-gray-500 mb-4 block">Nuance</span>
                            <div className="flex gap-4">
                                <ColorSwatch color="bg-[#1c1917]" active={activeColor === 0} onClick={() => setActiveColor(0)} />
                                <ColorSwatch color="bg-[#57534e]" active={activeColor === 1} onClick={() => setActiveColor(1)} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="pt-6 space-y-6">
                        <div className="flex gap-4">
                            ...
                            <button 
                                ...
                                className="flex-1 h-16 bg-white text-black hover:bg-yellow-500 hover:text-black transition-all duration-500 font-bold uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-3 shadow-[0_0_30px_rgba(255,255,255,0.1)] group"
                            >
                    ...
                    <div className="grid grid-cols-2 gap-4 py-8 border-t border-white/10">
                        <TechSpec icon={Gem} label="Matériau" value="Titane Pur (Grade 1)" />
                    ...
                    <div>
                        ...
                        <Accordion title="Livraison & Retours">
                             <p>Livraison offerte dans le monde entier via DHL Express.</p>
                             <p className="mt-2">Chaque commande est livrée dans un écrin signature.</p>
                        </Accordion>
                    </div>
